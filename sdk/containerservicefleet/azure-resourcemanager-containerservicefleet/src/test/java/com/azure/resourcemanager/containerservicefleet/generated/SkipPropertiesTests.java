...
public final class SkipPropertiesTests {
    ...
    public void testDeserialize() throws Exception {
        SkipProperties model
            = BinaryData.fromString("{\"targets\":[{\"type\":\"AfterStageWait\",\"name\":\"xzbinjeputt\"}]}")
                .toObject(SkipProperties.class);
        Assertions.assertEquals(TargetType.AFTER_STAGE_WAIT, model.targets().get(0).type());
        Assertions.assertEquals("xzbinjeputt", model.targets().get(0).name());
    }

    @org.junit.jupiter.api.Test
    public void testSerialize() throws Exception {
        SkipProperties model = new SkipProperties()
            .withTargets(Arrays.asList(new SkipTarget().withType(TargetType.AFTER_STAGE_WAIT).withName("xzbinjeputt")));

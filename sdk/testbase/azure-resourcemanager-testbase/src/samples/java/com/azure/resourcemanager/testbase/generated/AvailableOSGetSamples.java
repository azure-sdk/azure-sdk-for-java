// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.
// Code generated by Microsoft (R) AutoRest Code Generator.

package com.azure.resourcemanager.testbase.generated;

/**
 * Samples for AvailableOS Get.
 */
public final class AvailableOSGetSamples {
    /*
     * x-ms-original-file:
     * specification/testbase/resource-manager/Microsoft.TestBase/preview/2023-11-01-preview/examples/AvailableOSGet.
     * json
     */
    /**
     * Sample code: AvailableOSGet.
     * 
     * @param manager Entry point to TestBaseManager.
     */
    public static void availableOSGet(com.azure.resourcemanager.testbase.TestBaseManager manager) {
        manager.availableOS().getWithResponse("contoso-rg", "contoso-testBaseAccount", "Windows-10-2004",
            com.azure.core.util.Context.NONE);
    }
}

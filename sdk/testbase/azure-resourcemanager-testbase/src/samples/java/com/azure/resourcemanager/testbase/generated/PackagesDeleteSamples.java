// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.
// Code generated by Microsoft (R) AutoRest Code Generator.

package com.azure.resourcemanager.testbase.generated;

/**
 * Samples for Packages Delete.
 */
public final class PackagesDeleteSamples {
    /*
     * x-ms-original-file:
     * specification/testbase/resource-manager/Microsoft.TestBase/preview/2023-11-01-preview/examples/PackageDelete.json
     */
    /**
     * Sample code: PackageDelete.
     * 
     * @param manager Entry point to TestBaseManager.
     */
    public static void packageDelete(com.azure.resourcemanager.testbase.TestBaseManager manager) {
        manager.packages().delete("contoso-rg1", "contoso-testBaseAccount1", "contoso-package2",
            com.azure.core.util.Context.NONE);
    }
}

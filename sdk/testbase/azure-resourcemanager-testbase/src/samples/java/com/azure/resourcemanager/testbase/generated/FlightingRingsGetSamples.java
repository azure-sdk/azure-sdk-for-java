// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.
// Code generated by Microsoft (R) AutoRest Code Generator.

package com.azure.resourcemanager.testbase.generated;

/**
 * Samples for FlightingRings Get.
 */
public final class FlightingRingsGetSamples {
    /*
     * x-ms-original-file:
     * specification/testbase/resource-manager/Microsoft.TestBase/preview/2023-11-01-preview/examples/FlightingRingGet.
     * json
     */
    /**
     * Sample code: FlightingRingGet.
     * 
     * @param manager Entry point to TestBaseManager.
     */
    public static void flightingRingGet(com.azure.resourcemanager.testbase.TestBaseManager manager) {
        manager.flightingRings().getWithResponse("contoso-rg", "contoso-testBaseAccount", "Insider-Beta-Channel",
            com.azure.core.util.Context.NONE);
    }
}

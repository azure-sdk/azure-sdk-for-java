// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.
// Code generated by Microsoft (R) AutoRest Code Generator.

package com.azure.resourcemanager.logic.implementation;

import com.azure.resourcemanager.logic.fluent.models.WorkflowExportResultInner;
import com.azure.resourcemanager.logic.models.ContentLink;
import com.azure.resourcemanager.logic.models.ExportDetail;
import com.azure.resourcemanager.logic.models.WorkflowExportResult;
import java.util.Collections;
import java.util.List;

public final class WorkflowExportResultImpl implements WorkflowExportResult {
    private WorkflowExportResultInner innerObject;

    private final com.azure.resourcemanager.logic.LogicManager serviceManager;

    WorkflowExportResultImpl(
        WorkflowExportResultInner innerObject, com.azure.resourcemanager.logic.LogicManager serviceManager) {
        this.innerObject = innerObject;
        this.serviceManager = serviceManager;
    }

    public List<ExportDetail> details() {
        List<ExportDetail> inner = this.innerModel().details();
        if (inner != null) {
            return Collections.unmodifiableList(inner);
        } else {
            return Collections.emptyList();
        }
    }

    public ContentLink packageLink() {
        return this.innerModel().packageLink();
    }

    public WorkflowExportResultInner innerModel() {
        return this.innerObject;
    }

    private com.azure.resourcemanager.logic.LogicManager manager() {
        return this.serviceManager;
    }
}

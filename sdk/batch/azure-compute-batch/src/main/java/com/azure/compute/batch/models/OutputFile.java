...
public final class OutputFile {
     ...
     * matches any single character, [abc] matches one character in the brackets, and [a-c] matches one character in
     * the range. Brackets can include a negation to match any character not specified (for example [!abc] matches any
     ...
     * matches any file that does not start in '.' and ends with .txt in the Task working directory or any
     * subdirectory. If the filename contains a wildcard character it can be escaped using brackets (for example abc[*]
     * would match a file named abc*). Note that both \ and / are treated as directory separators on Windows, but only
     * / is on Linux. Environment variables (%var% on Windows or $var on Linux) are expanded prior to the pattern being
     * applied.
     * 

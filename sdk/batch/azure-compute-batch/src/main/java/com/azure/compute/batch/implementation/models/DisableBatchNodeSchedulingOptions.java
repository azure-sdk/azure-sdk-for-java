// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.
// Code generated by Microsoft (R) TypeSpec Code Generator.

package com.azure.compute.batch.implementation.models;

import com.azure.compute.batch.models.BatchNodeDisableSchedulingContent;
import com.azure.core.annotation.Fluent;
import com.azure.core.annotation.Generated;

/**
 * Optional parameters for Disable Node Scheduling operation.
 */
@Fluent
public final class DisableBatchNodeSchedulingOptions {
    /*
     * The maximum time that the server can spend processing the request, in seconds. The default is 30 seconds. If the
     * value is larger than 30, the default will be used instead.".
     */
    @Generated
    private Integer timeOutInSeconds;

    /*
     * The options to use for disabling scheduling on the Compute Node.
     */
    @Generated
    private BatchNodeDisableSchedulingContent parameters;

    /**
     * Creates an instance of DisableBatchNodeSchedulingOptions class.
     */
    @Generated
    public DisableBatchNodeSchedulingOptions() {
    }

    /**
     * Get the timeOutInSeconds property: The maximum time that the server can spend processing the request, in seconds.
     * The default is 30 seconds. If the value is larger than 30, the default will be used instead.".
     * 
     * @return the timeOutInSeconds value.
     */
    @Generated
    public Integer getTimeOutInSeconds() {
        return this.timeOutInSeconds;
    }

    /**
     * Set the timeOutInSeconds property: The maximum time that the server can spend processing the request, in seconds.
     * The default is 30 seconds. If the value is larger than 30, the default will be used instead.".
     * 
     * @param timeOutInSeconds the timeOutInSeconds value to set.
     * @return the DisableBatchNodeSchedulingOptions object itself.
     */
    @Generated
    public DisableBatchNodeSchedulingOptions setTimeOutInSeconds(Integer timeOutInSeconds) {
        this.timeOutInSeconds = timeOutInSeconds;
        return this;
    }

    /**
     * Get the parameters property: The options to use for disabling scheduling on the Compute Node.
     * 
     * @return the parameters value.
     */
    @Generated
    public BatchNodeDisableSchedulingContent getParameters() {
        return this.parameters;
    }

    /**
     * Set the parameters property: The options to use for disabling scheduling on the Compute Node.
     * 
     * @param parameters the parameters value to set.
     * @return the DisableBatchNodeSchedulingOptions object itself.
     */
    @Generated
    public DisableBatchNodeSchedulingOptions setParameters(BatchNodeDisableSchedulingContent parameters) {
        this.parameters = parameters;
        return this;
    }
}

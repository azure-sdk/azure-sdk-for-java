...
public final class SqlServerInstancesClientImpl implements SqlServerInstancesClient {
    ...
    private Mono<PagedResponse<List<String>>> getTelemetrySinglePageAsync(String resourceGroupName,
        ...
        return FluxUtil.withContext(context -> {
            Mono<Response<Flux<ByteBuffer>>> mono = service.getTelemetry(this.client.getEndpoint(),
                this.client.getSubscriptionId(), resourceGroupName, sqlServerInstanceName, this.client.getApiVersion(),
                sqlServerInstanceTelemetryRequest, accept, context).cache();
        ...
        }).<PagedResponse<List<String>>>map(res -> new PagedResponseBase<>(res.getT1().getRequest(),
            res.getT1().getStatusCode(), res.getT1().getHeaders(), res.getT2().rows(), res.getT2().nextLink(), null))
            .contextWrite(context -> context.putAll(FluxUtil.toReactorContext(this.client.getContext()).readOnly()));
    }
    ...
    private Mono<PagedResponse<List<String>>> postInstanceTelemetrySinglePageAsync(String nextLink, Context context) {
        ...
        context = this.client.mergeContext(context);
        return service.postInstanceTelemetry(nextLink, context).map(res -> new PagedResponseBase<>(res.getRequest(),
            res.getStatusCode(), res.getHeaders(), res.getValue().rows(), res.getValue().nextLink(), null));
    }
}

...
public final class RouterJob {
    ...
    RouterJob(RouterJobInternal internal) {
        ...
        requestedWorkerSelectors =
                internal.getRequestedWorkerSelectors().stream()
                        .map(ws -> LabelSelectorAdapter.convertWorkerSelectorToPublic(ws))
                        .collect(Collectors.toList());
        attachedWorkerSelectors =
                internal.getAttachedWorkerSelectors().stream()
                        .map(ws -> LabelSelectorAdapter.convertWorkerSelectorToPublic(ws))
                        .collect(Collectors.toList());
        assignments =
                internal.getAssignments().entrySet().stream()
        ...
        setLabels(
                internal.getLabels().entrySet().stream()
                        .collect(
                                Collectors.toMap(
                                        Map.Entry::getKey,
        ...
        setClassificationPolicyId(internal.getClassificationPolicyId());
        setTags(
                internal.getTags().entrySet().stream()
                        .collect(
                                Collectors.toMap(
                                        Map.Entry::getKey,
                                        entry -> RouterValueConstructorProxy.create(entry.getValue()))));

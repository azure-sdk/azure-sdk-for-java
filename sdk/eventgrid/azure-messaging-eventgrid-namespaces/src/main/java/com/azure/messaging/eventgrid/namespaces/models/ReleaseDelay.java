// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.
// Code generated by Microsoft (R) TypeSpec Code Generator.
package com.azure.messaging.eventgrid.namespaces.models;

import com.azure.core.annotation.Generated;
import com.azure.core.util.ExpandableStringEnum;
import java.util.Collection;

/**
 * Supported delays for release operation.
 */
public final class ReleaseDelay extends ExpandableStringEnum<ReleaseDelay> {

    /**
     * Release the event after 0 seconds.
     */
    @Generated
    public static final ReleaseDelay BY0_SECONDS = fromString("0");

    /**
     * Release the event after 10 seconds.
     */
    @Generated
    public static final ReleaseDelay BY10_SECONDS = fromString("10");

    /**
     * Release the event after 60 seconds.
     */
    @Generated
    public static final ReleaseDelay BY60_SECONDS = fromString("60");

    /**
     * Release the event after 600 seconds.
     */
    @Generated
    public static final ReleaseDelay BY600_SECONDS = fromString("600");

    /**
     * Release the event after 3600 seconds.
     */
    @Generated
    public static final ReleaseDelay BY3600_SECONDS = fromString("3600");

    /**
     * Creates a new instance of ReleaseDelay value.
     *
     * @deprecated Use the {@link #fromString(String)} factory method.
     */
    @Generated
    @Deprecated
    public ReleaseDelay() {
    }

    /**
     * Creates or finds a ReleaseDelay from its string representation.
     *
     * @param name a name to look for.
     * @return the corresponding ReleaseDelay.
     */
    @Generated
    public static ReleaseDelay fromString(String name) {
        return fromString(name, ReleaseDelay.class);
    }

    /**
     * Gets known ReleaseDelay values.
     *
     * @return known ReleaseDelay values.
     */
    @Generated
    public static Collection<ReleaseDelay> values() {
        return values(ReleaseDelay.class);
    }
}

...
public final class RaiPoliciesCreateOrUpdateSamples {
    ...
    public static void putRaiPolicy(com.azure.resourcemanager.cognitiveservices.CognitiveServicesManager manager) {
        manager.raiPolicies().define("raiPolicyName").withExistingAccount("resourceGroupName", "accountName").withProperties(new RaiPolicyProperties().withBasePolicyName("112").withContentFilters(Arrays.asList(new RaiPolicyContentFilter().withName("hate").withAllowedContentLevel(AllowedContentLevel.LOW).withBlocking(true), new RaiPolicyContentFilter().withName("sexual").withAllowedContentLevel(AllowedContentLevel.LOW), new RaiPolicyContentFilter().withName("violence").withEnabled(false), new RaiPolicyContentFilter().withName("DefaultHateSpeechBlockList").withEnabled(false)))).create();
    }
}
```

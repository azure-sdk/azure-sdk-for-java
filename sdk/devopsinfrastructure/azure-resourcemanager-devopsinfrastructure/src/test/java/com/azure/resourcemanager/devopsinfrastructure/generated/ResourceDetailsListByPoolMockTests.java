...
public final class ResourceDetailsListByPoolMockTests {
    ...
    public void testListByPool() throws Exception {
        String responseStr
            = "{\"value\":[{\"properties\":{\"status\":\"PendingReturn\",\"image\":\"su\",\"imageVersion\":\"arm\"},\"id\":\"dmjsjqb\",\"name\":\"hhyxxrw\",\"type\":\"yc\"}]}";
        ...
        PagedIterable<ResourceDetailsObject> response
            = manager.resourceDetails().listByPool("bdunygaeqid", "qfatpxllrxcyjm", com.azure.core.util.Context.NONE);

        Assertions.assertEquals(ResourceStatus.PENDING_RETURN, response.iterator().next().properties().status());
        Assertions.assertEquals("su", response.iterator().next().properties().image());
        Assertions.assertEquals("arm", response.iterator().next().properties().imageVersion());
    }
}

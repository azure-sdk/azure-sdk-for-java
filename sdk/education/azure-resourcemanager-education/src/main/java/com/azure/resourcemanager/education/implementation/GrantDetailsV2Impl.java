// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.
// Code generated by Microsoft (R) AutoRest Code Generator.

package com.azure.resourcemanager.education.implementation;

import com.azure.core.management.SystemData;
import com.azure.resourcemanager.education.fluent.models.Amount;
import com.azure.resourcemanager.education.fluent.models.GrantDetailsV2Inner;
import com.azure.resourcemanager.education.models.GrantDetailsV2;
import com.azure.resourcemanager.education.models.GrantStatus;
import com.azure.resourcemanager.education.models.GrantType;
import java.time.OffsetDateTime;

public final class GrantDetailsV2Impl implements GrantDetailsV2 {
    private GrantDetailsV2Inner innerObject;

    private final com.azure.resourcemanager.education.EducationManager serviceManager;

    GrantDetailsV2Impl(
        GrantDetailsV2Inner innerObject, com.azure.resourcemanager.education.EducationManager serviceManager) {
        this.innerObject = innerObject;
        this.serviceManager = serviceManager;
    }

    public String id() {
        return this.innerModel().id();
    }

    public String name() {
        return this.innerModel().name();
    }

    public String type() {
        return this.innerModel().type();
    }

    public SystemData systemData() {
        return this.innerModel().systemData();
    }

    public String displayName() {
        return this.innerModel().displayName();
    }

    public Amount offerCap() {
        return this.innerModel().offerCap();
    }

    public OffsetDateTime effectiveDate() {
        return this.innerModel().effectiveDate();
    }

    public GrantType offerType() {
        return this.innerModel().offerType();
    }

    public OffsetDateTime expirationDate() {
        return this.innerModel().expirationDate();
    }

    public GrantStatus status() {
        return this.innerModel().status();
    }

    public Amount allocatedBudget() {
        return this.innerModel().allocatedBudget();
    }

    public GrantDetailsV2Inner innerModel() {
        return this.innerObject;
    }

    private com.azure.resourcemanager.education.EducationManager manager() {
        return this.serviceManager;
    }
}

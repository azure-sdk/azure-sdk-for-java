// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.
// Code generated by Microsoft (R) TypeSpec Code Generator.

package com.azure.resourcemanager.hybridconnectivity.generated;

import com.azure.core.util.BinaryData;
import com.azure.resourcemanager.hybridconnectivity.models.SolutionSettings;
import com.azure.resourcemanager.hybridconnectivity.models.SolutionTypeSettings;
import java.util.HashMap;
import java.util.Map;
import org.junit.jupiter.api.Assertions;

public final class SolutionTypeSettingsTests {
    @org.junit.jupiter.api.Test
    public void testDeserialize() throws Exception {
        SolutionTypeSettings model = BinaryData.fromString(
            "{\"solutionType\":\"icndvkaozwyifty\",\"solutionSettings\":{\"\":{\"xolniwpwcukjfk\":\"rokft\",\"lwckbasyypnddhs\":\"iawxklry\"}}}")
            .toObject(SolutionTypeSettings.class);
        Assertions.assertEquals("icndvkaozwyifty", model.solutionType());
    }

    @org.junit.jupiter.api.Test
    public void testSerialize() throws Exception {
        SolutionTypeSettings model = new SolutionTypeSettings().withSolutionType("icndvkaozwyifty")
            .withSolutionSettings(new SolutionSettings().withAdditionalProperties(mapOf()));
        model = BinaryData.fromObject(model).toObject(SolutionTypeSettings.class);
        Assertions.assertEquals("icndvkaozwyifty", model.solutionType());
    }

    // Use "Map.of" if available
    @SuppressWarnings("unchecked")
    private static <T> Map<String, T> mapOf(Object... inputs) {
        Map<String, T> map = new HashMap<>();
        for (int i = 0; i < inputs.length; i += 2) {
            String key = (String) inputs[i];
            T value = (T) inputs[i + 1];
            map.put(key, value);
        }
        return map;
    }
}

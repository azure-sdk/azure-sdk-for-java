// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.
// Code generated by Microsoft (R) AutoRest Code Generator.

package com.azure.resourcemanager.devtestlabs.generated;

/**
 * Samples for BastionHosts CreateOrUpdate.
 */
public final class BastionHostsCreateOrUpdateSamples {
    /*
     * x-ms-original-file: specification/devtestlabs/resource-manager/Microsoft.DevTestLab/stable/2021-09-01/examples/
     * BastionHosts_CreateOrUpdate.json
     */
    /**
     * Sample code: BastionHosts_CreateOrUpdate.
     * 
     * @param manager Entry point to DevTestLabsManager.
     */
    public static void bastionHostsCreateOrUpdate(com.azure.resourcemanager.devtestlabs.DevTestLabsManager manager) {
        manager.bastionHosts().define("{bastionHostName}").withRegion("westcentralus")
            .withExistingVirtualnetwork("resourceGroupName", "exampleLab", "dtlexampleLab").create();
    }
}

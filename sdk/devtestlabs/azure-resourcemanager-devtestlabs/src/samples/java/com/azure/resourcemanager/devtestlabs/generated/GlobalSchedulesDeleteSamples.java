// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.
// Code generated by Microsoft (R) AutoRest Code Generator.

package com.azure.resourcemanager.devtestlabs.generated;

/**
 * Samples for GlobalSchedules Delete.
 */
public final class GlobalSchedulesDeleteSamples {
    /*
     * x-ms-original-file:
     * specification/devtestlabs/resource-manager/Microsoft.DevTestLab/stable/2021-09-01/examples/GlobalSchedules_Delete
     * .json
     */
    /**
     * Sample code: GlobalSchedules_Delete.
     * 
     * @param manager Entry point to DevTestLabsManager.
     */
    public static void globalSchedulesDelete(com.azure.resourcemanager.devtestlabs.DevTestLabsManager manager) {
        manager.globalSchedules().deleteByResourceGroupWithResponse("resourceGroupName", "labvmautostart",
            com.azure.core.util.Context.NONE);
    }
}

// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.
// Code generated by Microsoft (R) AutoRest Code Generator.

package com.azure.resourcemanager.devtestlabs.generated;

/**
 * Samples for Formulas Delete.
 */
public final class FormulasDeleteSamples {
    /*
     * x-ms-original-file:
     * specification/devtestlabs/resource-manager/Microsoft.DevTestLab/stable/2021-09-01/examples/Formulas_Delete.json
     */
    /**
     * Sample code: Formulas_Delete.
     * 
     * @param manager Entry point to DevTestLabsManager.
     */
    public static void formulasDelete(com.azure.resourcemanager.devtestlabs.DevTestLabsManager manager) {
        manager.formulas().deleteWithResponse("resourceGroupName", "exampleLab", "{formulaName}",
            com.azure.core.util.Context.NONE);
    }
}

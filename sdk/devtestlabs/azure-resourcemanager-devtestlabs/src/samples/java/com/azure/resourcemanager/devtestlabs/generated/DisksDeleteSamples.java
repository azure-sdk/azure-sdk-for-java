// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.
// Code generated by Microsoft (R) AutoRest Code Generator.

package com.azure.resourcemanager.devtestlabs.generated;

/**
 * Samples for Disks Delete.
 */
public final class DisksDeleteSamples {
    /*
     * x-ms-original-file:
     * specification/devtestlabs/resource-manager/Microsoft.DevTestLab/stable/2021-09-01/examples/Disks_Delete.json
     */
    /**
     * Sample code: Disks_Delete.
     * 
     * @param manager Entry point to DevTestLabsManager.
     */
    public static void disksDelete(com.azure.resourcemanager.devtestlabs.DevTestLabsManager manager) {
        manager.disks().delete("resourceGroupName", "exampleLab", "{userId}", "diskName",
            com.azure.core.util.Context.NONE);
    }
}

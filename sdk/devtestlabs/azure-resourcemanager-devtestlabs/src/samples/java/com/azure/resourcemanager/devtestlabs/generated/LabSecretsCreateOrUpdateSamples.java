// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.
// Code generated by Microsoft (R) AutoRest Code Generator.

package com.azure.resourcemanager.devtestlabs.generated;

/**
 * Samples for LabSecrets CreateOrUpdate.
 */
public final class LabSecretsCreateOrUpdateSamples {
    /*
     * x-ms-original-file: specification/devtestlabs/resource-manager/Microsoft.DevTestLab/stable/2021-09-01/examples/
     * LabSecrets_CreateOrUpdate.json
     */
    /**
     * Sample code: LabSecrets_CreateOrUpdate.
     * 
     * @param manager Entry point to DevTestLabsManager.
     */
    public static void labSecretsCreateOrUpdate(com.azure.resourcemanager.devtestlabs.DevTestLabsManager manager) {
        manager.labSecrets().define("{secretName}").withRegion("westcentralus")
            .withExistingLab("resourceGroupName", "exampleLab").withValue("{secretValue}").withEnabledForArtifacts(true)
            .withEnabledForVmCreation(true).withEnabledForArmEnvironments(true).create();
    }
}

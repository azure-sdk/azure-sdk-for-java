// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.
// Code generated by Microsoft (R) AutoRest Code Generator.

package com.azure.resourcemanager.devtestlabs.generated;

/**
 * Samples for ArtifactSources Get.
 */
public final class ArtifactSourcesGetSamples {
    /*
     * x-ms-original-file:
     * specification/devtestlabs/resource-manager/Microsoft.DevTestLab/stable/2021-09-01/examples/ArtifactSources_Get.
     * json
     */
    /**
     * Sample code: ArtifactSources_Get.
     * 
     * @param manager Entry point to DevTestLabsManager.
     */
    public static void artifactSourcesGet(com.azure.resourcemanager.devtestlabs.DevTestLabsManager manager) {
        manager.artifactSources().getWithResponse("resourceGroupName", "exampleLab", "artifactSourceName", null,
            com.azure.core.util.Context.NONE);
    }
}

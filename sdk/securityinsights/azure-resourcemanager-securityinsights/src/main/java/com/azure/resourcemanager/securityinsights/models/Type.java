// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.
// Code generated by Microsoft (R) AutoRest Code Generator.

package com.azure.resourcemanager.securityinsights.models;

import com.azure.core.util.ExpandableStringEnum;
import com.fasterxml.jackson.annotation.JsonCreator;
import java.util.Collection;

/** The type of the summary. */
public final class Type extends ExpandableStringEnum<Type> {
    /** Static value dynamic-summary for Type. */
    public static final Type DYNAMIC_SUMMARY = fromString("dynamic-summary");

    /** Static value dynamic-summary-item for Type. */
    public static final Type DYNAMIC_SUMMARY_ITEM = fromString("dynamic-summary-item");

    /**
     * Creates or finds a Type from its string representation.
     *
     * @param name a name to look for.
     * @return the corresponding Type.
     */
    @JsonCreator
    public static Type fromString(String name) {
        return fromString(name, Type.class);
    }

    /**
     * Gets known Type values.
     *
     * @return known Type values.
     */
    public static Collection<Type> values() {
        return values(Type.class);
    }
}

...
public final class EmergingIssuesImpl implements EmergingIssues {
    ...
    public Response<EmergingIssuesGetResultAutoGenerated> getWithResponse(Context context) {
        Response<EmergingIssuesGetResultAutoGeneratedInner> inner = this.serviceClient().getWithResponse(context);
        if (inner != null) {
            return new SimpleResponse<>(
                inner.getRequest(),
                inner.getStatusCode(),
                inner.getHeaders(),
                new EmergingIssuesGetResultAutoGeneratedImpl(inner.getValue(), this.manager()));
        } else {
            return null;
        }
    }

    public EmergingIssuesGetResultAutoGenerated get() {
        EmergingIssuesGetResultAutoGeneratedInner inner = this.serviceClient().get();
        if (inner != null) {
            return new EmergingIssuesGetResultAutoGeneratedImpl(inner, this.manager());
        } else {
            return null;
        }
    }

    public PagedIterable<EmergingIssuesGetResultAutoGenerated> list() {
        PagedIterable<EmergingIssuesGetResultAutoGeneratedInner> inner = this.serviceClient().list();
        return Utils.mapPage(inner, inner1 -> new EmergingIssuesGetResultAutoGeneratedImpl(inner1, this.manager()));
    }

    public PagedIterable<EmergingIssuesGetResultAutoGenerated> list(Context context) {
        PagedIterable<EmergingIssuesGetResultAutoGeneratedInner> inner = this.serviceClient().list(context);
        return Utils.mapPage(inner, inner1 -> new EmergingIssuesGetResultAutoGeneratedImpl(inner1, this.manager()));
    }

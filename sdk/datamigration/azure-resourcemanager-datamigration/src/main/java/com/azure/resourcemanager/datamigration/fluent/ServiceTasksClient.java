...
public interface ServiceTasksClient {
     ...
     * (classic). This method returns a list of service level tasks owned by a service resource. Some tasks may have a
     ...
     *
     * <p>The service tasks resource is a nested, proxy-only resource representing work performed by a DMS (classic)
     * instance. The PUT method creates a new service task or updates an existing one, although since service tasks have
     * no mutable custom properties, there is little reason to update an existing one. Database Migration Service
     * (classic) - SQL scenarios are on a deprecation path and will be retired on 15 March 2026 for all customers.
     * Please migrate to Azure SQL database services by using the latest Azure Database Migration Service version which

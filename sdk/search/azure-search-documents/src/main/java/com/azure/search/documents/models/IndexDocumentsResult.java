...
public final class IndexDocumentsResult implements JsonSerializable<IndexDocumentsResult> {
    ...
    public static IndexDocumentsResult fromJson(JsonReader jsonReader) throws IOException {
        return jsonReader.readObject(reader -> {
            IndexDocumentsResult deserializedIndexDocumentsResult = new IndexDocumentsResult();
            while (reader.nextToken() != JsonToken.END_OBJECT) {
                String fieldName = reader.getFieldName();
                reader.nextToken();

                if ("value".equals(fieldName)) {
                    List<IndexingResult> results = reader.readArray(reader1 -> IndexingResult.fromJson(reader1));
                    deserializedIndexDocumentsResult.results = results;
                } else {
                    reader.skipChildren();
                }
            }

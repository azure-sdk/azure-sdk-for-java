...
public final class RuntimeVersionsImpl implements RuntimeVersions {
    ...
    public AvailableRuntimeVersionsAutoGenerated listRuntimeVersions() {
        AvailableRuntimeVersionsAutoGeneratedInner inner = this.serviceClient().listRuntimeVersions();
        if (inner != null) {
            return new AvailableRuntimeVersionsAutoGeneratedImpl(inner, this.manager());
        } else {
            return null;
        }
    }

    public Response<AvailableRuntimeVersionsAutoGenerated> listRuntimeVersionsWithResponse(Context context) {
        Response<AvailableRuntimeVersionsAutoGeneratedInner> inner =
            this.serviceClient().listRuntimeVersionsWithResponse(context);
        if (inner != null) {
            return new SimpleResponse<>(
                inner.getRequest(),
                inner.getStatusCode(),
                inner.getHeaders(),
                new AvailableRuntimeVersionsAutoGeneratedImpl(inner.getValue(), this.manager()));
        } else {
            return null;
        }
    }

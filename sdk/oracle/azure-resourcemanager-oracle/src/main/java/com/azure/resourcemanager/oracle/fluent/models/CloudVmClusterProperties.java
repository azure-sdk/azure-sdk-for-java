...
public final class CloudVmClusterProperties {
     ...
     * RECO storage (database redo logs, archive logs, and recovery manager backups). Accepted values are 35, 40, 60
     * and 80. The default is 80 percent assigned to DATA storage. See [Storage
    ...
    /*
     * The type of redundancy configured for the cloud Vm cluster. NORMAL is 2-way redundancy. HIGH is 3-way
     * redundancy.
     */
    ...
    /*
     * The virtual IP (VIP) addresses associated with the cloud VM cluster. The Cluster Ready Services (CRS) creates
     * and maintains one VIP address for each node in the Exadata Cloud Service instance to enable failover. If one
     * node fails, the VIP is reassigned to another active node in the cluster. **Note:** For a single-node DB system,
     * this list is empty.
     */
    ...
    private CloudVmClusterProvisioningState provisioningState;
    ...
    /*
     * Indicates user preferences for the various diagnostic collection options for the VM cluster/Cloud VM
     * cluster/VMBM DBCS.
     */
    ...
    public void validate() {
        if (hostname() == null) {
            throw LOGGER.logExceptionAsError(
                new IllegalArgumentException("Missing required property hostname in model CloudVmClusterProperties"));
        }
        ...
        if (giVersion() == null) {
            throw LOGGER.logExceptionAsError(
                new IllegalArgumentException("Missing required property giVersion in model CloudVmClusterProperties"));
        }
        if (subnetId() == null) {
            throw LOGGER.logExceptionAsError(
                new IllegalArgumentException("Missing required property subnetId in model CloudVmClusterProperties"));
        }

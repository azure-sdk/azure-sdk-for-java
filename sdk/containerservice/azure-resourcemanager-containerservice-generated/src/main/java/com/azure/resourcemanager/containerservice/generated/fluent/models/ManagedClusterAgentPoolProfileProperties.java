...
public class ManagedClusterAgentPoolProfileProperties {
    ...
    /*
     * Specifies the OS SKU used by the agent pool. If not specified, the
     * default is Ubuntu if OSType=Linux or Windows2019 if OSType=Windows. And
     * the default Windows OSSKU will be changed to Windows2022 after
     ...
     * <major.minor.patch> and <major.minor> are supported. When <major.minor>
     * is specified, the latest supported patch version is chosen
     * automatically. Updating the agent pool with the same <major.minor> once
     * it has been created will not trigger an upgrade, even if a newer patch
     * version is available. As a best practice, you should upgrade all node
     * pools in an AKS cluster to the same Kubernetes version. The node pool
     * version must have the same major version as the control plane. The node
     * pool minor version must be within two minor versions of the control
     * plane version. The node pool version cannot be greater than the control
    ...
    /*
     * The version of Kubernetes running on the Agent Pool. If
     * orchestratorVersion was a fully specified version <major.minor.patch>,
     ...
     * version being used.
     */
    @JsonProperty(value = "currentOrchestratorVersion")
    ...
    /*
     * Whether to enable Custom CA Trust feature. When set to true, AKS deploys
     * a daemonset and host services to sync custom certificate authorities
     * from a user-provided config map into node trust stores. Defaults to
     * false.
     */
     ...
     * @return the ManagedClusterAgentPoolProfileProperties object itself.
     */
    public ManagedClusterAgentPoolProfileProperties withCurrentOrchestratorVersion(String currentOrchestratorVersion) {
    ...
    /**
     * Get the enableCustomCATrust property: Whether to enable Custom CA Trust feature. When set to true, AKS deploys a
    ...
    /**
     * Set the enableCustomCATrust property: Whether to enable Custom CA Trust feature. When set to true, AKS deploys a
     * daemonset and host services to sync custom certificate authorities from a user-provided config map into node
     * trust stores. Defaults to false.
     *

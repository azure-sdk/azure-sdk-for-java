...
public interface AgentPool {
    ...
    interface DefinitionStages {
        ...
        interface WithOrchestratorVersion {
             ...
             *     Updating the cluster with the same &lt;major.minor&gt; once it has been created (e.g. 1.14.x -&gt;
             *     1.14) will not trigger an upgrade, even if a newer patch version is available. As a best practice,
             *     you should upgrade all node pools in an AKS cluster to the same Kubernetes version. The node pool
             *     version must have the same major version as the control plane. The node pool minor version must be
             *     within two minor versions of the control plane version. The node pool version cannot be greater than

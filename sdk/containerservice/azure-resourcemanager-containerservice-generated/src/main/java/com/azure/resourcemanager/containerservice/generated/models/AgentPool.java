...
public interface AgentPool {
    ...
    /**
     * Gets the orchestratorVersion property: The version of Kubernetes running on the Agent Pool.
     *
     * <p>As a best practice, you should upgrade all node pools in an AKS cluster to the same Kubernetes version. The
     * node pool version must have the same major version as the control plane. The node pool minor version must be
     * within two minor versions of the control plane version. The node pool version cannot be greater than the control
    ...
    interface DefinitionStages {
        ...
        interface WithOrchestratorVersion {
            /**
             * Specifies the orchestratorVersion property: The version of Kubernetes running on the Agent Pool.
             *
             * <p>As a best practice, you should upgrade all node pools in an AKS cluster to the same Kubernetes
             * version. The node pool version must have the same major version as the control plane. The node pool minor
             * version must be within two minor versions of the control plane version. The node pool version cannot be
             * greater than the control plane version. For more information see [upgrading a node
             * pool](https://docs.microsoft.com/azure/aks/use-multiple-node-pools#upgrade-a-node-pool)..
             *
             * @param orchestratorVersion The version of Kubernetes running on the Agent Pool.
             *     <p>As a best practice, you should upgrade all node pools in an AKS cluster to the same Kubernetes
             *     version. The node pool version must have the same major version as the control plane. The node pool
             *     minor version must be within two minor versions of the control plane version. The node pool version

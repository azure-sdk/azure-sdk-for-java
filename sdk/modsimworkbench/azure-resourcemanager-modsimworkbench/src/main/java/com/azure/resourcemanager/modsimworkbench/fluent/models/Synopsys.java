// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.
// Code generated by Microsoft (R) AutoRest Code Generator.

package com.azure.resourcemanager.modsimworkbench.fluent.models;

import com.azure.core.annotation.Fluent;
import com.fasterxml.jackson.annotation.JsonProperty;
import java.util.List;

/** Synopsys Common Licensing. */
@Fluent
public final class Synopsys {
    /*
     * Switch to show if License Service is activated.
     */
    @JsonProperty(value = "enable")
    private Boolean enable;

    /*
     * License Service server.
     */
    @JsonProperty(value = "server")
    private String server;

    /*
     * Virtual machine universal unique identifier of ModSim Workbench chamber license servers used by FlexNet
     * Licensing Service.
     */
    @JsonProperty(value = "vmUuid")
    private List<String> vmUuid;

    /** Creates an instance of Synopsys class. */
    public Synopsys() {
    }

    /**
     * Get the enable property: Switch to show if License Service is activated.
     *
     * @return the enable value.
     */
    public Boolean enable() {
        return this.enable;
    }

    /**
     * Set the enable property: Switch to show if License Service is activated.
     *
     * @param enable the enable value to set.
     * @return the Synopsys object itself.
     */
    public Synopsys withEnable(Boolean enable) {
        this.enable = enable;
        return this;
    }

    /**
     * Get the server property: License Service server.
     *
     * @return the server value.
     */
    public String server() {
        return this.server;
    }

    /**
     * Set the server property: License Service server.
     *
     * @param server the server value to set.
     * @return the Synopsys object itself.
     */
    public Synopsys withServer(String server) {
        this.server = server;
        return this;
    }

    /**
     * Get the vmUuid property: Virtual machine universal unique identifier of ModSim Workbench chamber license servers
     * used by FlexNet Licensing Service.
     *
     * @return the vmUuid value.
     */
    public List<String> vmUuid() {
        return this.vmUuid;
    }

    /**
     * Set the vmUuid property: Virtual machine universal unique identifier of ModSim Workbench chamber license servers
     * used by FlexNet Licensing Service.
     *
     * @param vmUuid the vmUuid value to set.
     * @return the Synopsys object itself.
     */
    public Synopsys withVmUuid(List<String> vmUuid) {
        this.vmUuid = vmUuid;
        return this;
    }

    /**
     * Validates the instance.
     *
     * @throws IllegalArgumentException thrown if the instance is not valid.
     */
    public void validate() {
    }
}

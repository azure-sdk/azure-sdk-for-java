...
public final class SnapshotResourcesImpl implements SnapshotResources {
    private static final ClientLogger LOGGER = new ClientLogger(SnapshotResourcesImpl.class);

    private final SnapshotResourcesClient innerClient;

    private final com.azure.resourcemanager.appcomplianceautomation.AppComplianceAutomationManager serviceManager;

    public SnapshotResourcesImpl(SnapshotResourcesClient innerClient,
        com.azure.resourcemanager.appcomplianceautomation.AppComplianceAutomationManager serviceManager) {
        this.innerClient = innerClient;
        this.serviceManager = serviceManager;
    }

    public PagedIterable<SnapshotResource> listByReportResource(String reportName) {
        PagedIterable<SnapshotResourceInner> inner = this.serviceClient().listByReportResource(reportName);
        return Utils.mapPage(inner, inner1 -> new SnapshotResourceImpl(inner1, this.manager()));
    }

    public PagedIterable<SnapshotResource> listByReportResource(String reportName, String skipToken, Integer top,
        String select, String reportCreatorTenantId, String offerGuid, Context context) {
        PagedIterable<SnapshotResourceInner> inner = this.serviceClient().listByReportResource(reportName, skipToken,
            top, select, reportCreatorTenantId, offerGuid, context);
        return Utils.mapPage(inner, inner1 -> new SnapshotResourceImpl(inner1, this.manager()));
    }
    ...
    private SnapshotResourcesClient serviceClient() {

...
public final class OffersImpl implements Offers {
    ...
    public PagedIterable<Offer> list(String resourceUri, Context context) {
        PagedIterable<OfferInner> inner = this.serviceClient().list(resourceUri, context);
        return Utils.mapPage(inner, inner1 -> new OfferImpl(inner1, this.manager()));
    }
    ...
    public DiskAccessToken getAccessToken(String resourceUri, String offerId, AccessTokenRequest body) {
    ...
    public DiskAccessToken getAccessToken(
        String resourceUri, String offerId, AccessTokenRequest body, Context context) {
        DiskAccessTokenInner inner = this.serviceClient().getAccessToken(resourceUri, offerId, body, context);
        if (inner != null) {
            return new DiskAccessTokenImpl(inner, this.manager());
        } else {
            return null;
    ...
    public PagedIterable<Offer> search(String resourceUri) {
        PagedIterable<OfferInner> inner = this.serviceClient().search(resourceUri);
        return Utils.mapPage(inner, inner1 -> new OfferImpl(inner1, this.manager()));
    }

    public PagedIterable<Offer> search(String resourceUri, OfferSearchRequest body, Context context) {
        PagedIterable<OfferInner> inner = this.serviceClient().search(resourceUri, body, context);
        return Utils.mapPage(inner, inner1 -> new OfferImpl(inner1, this.manager()));
    }

// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.
// Code generated by Microsoft (R) AutoRest Code Generator.

package com.azure.resourcemanager.chaos.implementation;

import com.azure.resourcemanager.chaos.fluent.models.PostActionResultInner;
import com.azure.resourcemanager.chaos.models.PostActionResult;

public final class PostActionResultImpl implements PostActionResult {
    private PostActionResultInner innerObject;

    private final com.azure.resourcemanager.chaos.ChaosManager serviceManager;

    PostActionResultImpl(PostActionResultInner innerObject,
        com.azure.resourcemanager.chaos.ChaosManager serviceManager) {
        this.innerObject = innerObject;
        this.serviceManager = serviceManager;
    }

    public String name() {
        return this.innerModel().name();
    }

    public String statusUrl() {
        return this.innerModel().statusUrl();
    }

    public PostActionResultInner innerModel() {
        return this.innerObject;
    }

    private com.azure.resourcemanager.chaos.ChaosManager manager() {
        return this.serviceManager;
    }
}

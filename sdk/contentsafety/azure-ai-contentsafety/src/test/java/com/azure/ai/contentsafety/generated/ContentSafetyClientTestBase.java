...
import com.azure.ai.contentsafety.RaiPolicyClient;
import com.azure.ai.contentsafety.RaiPolicyClientBuilder;
...
class ContentSafetyClientTestBase extends TestProxyTestBase {
    ...
    protected RaiPolicyClient raiPolicyClient;
    ...
    protected void beforeTest() {
        ...
        RaiPolicyClientBuilder raiPolicyClientbuilder
            = new RaiPolicyClientBuilder().endpoint(Configuration.getGlobalConfiguration().get("ENDPOINT", "endpoint"))
                .httpClient(getHttpClientOrUsePlayback(getHttpClients().findFirst().orElse(null)))
                .httpLogOptions(new HttpLogOptions().setLogLevel(HttpLogDetailLevel.BASIC));
        if (getTestMode() == TestMode.PLAYBACK) {
            raiPolicyClientbuilder.credential(new MockTokenCredential());
        } else if (getTestMode() == TestMode.RECORD) {
            raiPolicyClientbuilder.addPolicy(interceptorManager.getRecordPolicy())
                .credential(new DefaultAzureCredentialBuilder().build());
        } else if (getTestMode() == TestMode.LIVE) {
            raiPolicyClientbuilder.credential(new DefaultAzureCredentialBuilder().build());
        }
        raiPolicyClient = raiPolicyClientbuilder.buildClient();

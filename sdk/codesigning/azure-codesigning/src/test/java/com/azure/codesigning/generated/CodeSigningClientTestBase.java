...
class CodeSigningClientTestBase extends TestProxyTestBase {
    ...
    protected void beforeTest() {
        CodeSigningClientBuilder codeSigningClientbuilder
            = new CodeSigningClientBuilder().region(Configuration.getGlobalConfiguration().get("REGION", "region"))
                .httpClient(HttpClient.createDefault())
                .httpLogOptions(new HttpLogOptions().setLogLevel(HttpLogDetailLevel.BASIC));
        if (getTestMode() == TestMode.PLAYBACK) {
            codeSigningClientbuilder.httpClient(interceptorManager.getPlaybackClient())
                .credential(request -> Mono.just(new AccessToken("this_is_a_token", OffsetDateTime.MAX)));
        } else if (getTestMode() == TestMode.RECORD) {
            codeSigningClientbuilder.addPolicy(interceptorManager.getRecordPolicy())
